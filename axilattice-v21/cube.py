...
class CubeEngine:
    ...
    def _compute_stats(self):
        """Pre-compute distribution stats per group for z-score anomaly detection."""
        self.conn.execute("""
            CREATE OR REPLACE TABLE axl_stats AS
            SELECT
                grain, dim_combo, measure,
                AVG(val_sum) AS val_mean,
                STDDEV(val_sum) AS val_stddev,
                MIN(val_sum) AS val_min,
                MAX(val_sum) AS val_max,
                COUNT(*) AS sample_size
            FROM axl_cube
            GROUP BY grain, dim_combo, measure
        """)
    ...
    def query_breakdown(self, dimension: str, measure: str,
        ...
        Includes delta_pct, rank_in_period, z_score for deep insight generation."""
        ...
        rows = self.conn.execute(f"""
            SELECT
                ...
                dl.rank_in_period,
                dl.prior_sum,
                st.val_mean AS group_mean,
                st.val_stddev AS group_stddev
            FROM {CUBE_TABLE} d
            LEFT JOIN axl_deltas dl
                USING (grain, period_key, dim_combo, dim_json, measure)
            LEFT JOIN axl_stats st
                ON d.grain = st.grain AND d.dim_combo = st.dim_combo AND d.measure = st.measure
            WHERE d.grain     = ?
              AND d.period_key = ?
              AND d.dim_combo = ?
              AND d.measure   = ?
        ...
        for r in rows:
            ...
            z_score = None
            if r[6] and r[6] > 0:
                z_score = (r[1] - r[5]) / r[6] if r[5] is not None else None
            result.append({
                "label": r[0], "value": r[1],
                "delta": r[2], "rank": r[3], "prior_sum": r[4],
                "group_mean": r[5], "group_stddev": r[6],
                "z_score": round(z_score, 2) if z_score is not None else None,
            })
        return result

    def query_filtered_breakdown(self, dimension: str, measure: str,
                                  filter_dim: str, filter_val: str,
                                  grain: str, period_key: Optional[str] = None) -> List[dict]:
        """Cross-dimensional filtering: breakdown dimension WHERE filter_dim = filter_val.
        Example: dimension="category", filter_dim="region", filter_val="South"
        → categories within South Region only."""
        pk = period_key or self._latest_period(grain)
        if not pk:
            return []
        combo_key = "|".join(sorted([dimension, filter_dim]))
        rows = self.conn.execute(f"""
            SELECT
                JSON_EXTRACT_STRING(d.dim_json, '$.{dimension}') AS label,
                d.val_sum AS value,
            ...
            WHERE d.grain     = ?
              ...
              AND d.dim_combo = ?
              AND d.measure   = ?
              AND JSON_EXTRACT_STRING(d.dim_json, '$.{filter_dim}') = ?
            ORDER BY d.val_sum DESC
        """, [grain, pk, combo_key, measure, filter_val]).fetchall()
    ...
    def query_trend(self, measure: str, grain: str,
        ...
        """Return total measure over last n periods with anomaly flags."""
        rows = self.conn.execute(f"""
            SELECT d.period_key, d.val_sum AS value, dl.delta_pct,
                   st.val_mean AS group_mean, st.val_stddev AS group_stddev
            FROM {CUBE_TABLE} d
            LEFT JOIN axl_deltas dl
                USING (grain, period_key, dim_combo, dim_json, measure)
            LEFT JOIN axl_stats st
        ...
        for r in reversed(rows):
            z_score = None
            if r[4] and r[4] > 0:
                z_score = (r[1] - r[3]) / r[4] if r[3] is not None else None
            result.append({
                "period": r[0], "value": r[1], "delta": r[2],
                "z_score": round(z_score, 2) if z_score is not None else None,
                "anomaly": abs(z_score) > 2 if z_score is not None else False,
            })
        return result
    ...
    def query_comparison(self, dimension: str, measure: str,
                         grain: str, period_key: Optional[str] = None) -> List[dict]:
        """Period comparison with change arrows and anomaly detection."""
        pk = period_key or self._latest_period(grain)
        if not pk:
            return []
        rows = self.conn.execute(f"""
        ...
        for r in rows:
            ...
            if r[6] and r[6] > 0:
                z_score = (r[1] - r[5]) / r[6] if r[5] is not None else None
            result.append({
                "label": r[0], "current_val": r[1], "prior_val": r[3],
                "delta": r[2], "rank": r[4],
                "z_score": round(z_score, 2) if z_score is not None else None,
                "anomaly": abs(z_score) > 2 if z_score is not None else False,
            })
    ...
    def query_anomaly(self, dimension: str, measure: str,
                      grain: str, period_key: Optional[str] = None,
                      threshold: float = 2.0) -> List[dict]:
        """Anomaly detection using pre-computed z-scores."""
        pk = period_key or self._latest_period(grain)
        if not pk:
            return []
        rows = self.conn.execute(f"""
        ...
        anomalies = []
        for r in rows:
            if r[0] is None or r[6] is None or r[6] == 0:
                continue
            z = (r[1] - r[5]) / r[6] if r[5] is not None else 0
            if abs(z) > threshold:
                severity = "critical" if abs(z) > 3 else "significant"
                anomalies.append({
                    "label": r[0], "value": r[1], "z_score": round(z, 2),
                    "delta": r[2], "prior_val": r[3], "rank": r[4],
                    "severity": severity, "direction": "above" if z > 0 else "below",
                })
        return sorted(anomalies, key=lambda x: abs(x["z_score"]), reverse=True)

    def query_growth(self, measure: str, grain: str) -> List[dict]:
        """Compute CAGR and total growth."""
        rows = self.conn.execute(f"""
            SELECT period_key, val_sum
            FROM {CUBE_TABLE}
            WHERE grain = ? AND dim_combo = '__total__' AND measure = ?
            ORDER BY period_key
        """, [grain, measure]).fetchall()
        ...
        return [{
            ...
            "periods": periods, "cagr": round(cagr * 100, 2),
            "total_growth_pct": round(total_growth, 2),
        }]

    def query_seasonality(self, measure: str, grain: str) -> List[dict]:
        ...
        for lag in range(2, min(len(values) // 2, 6)):
            corr = self._autocorr(values, lag)
            if corr > best_corr:
                best_corr = corr
                best_lag = lag
        if best_corr > 0.5:
            peaks = [i for i in range(1, len(values)-1) if values[i] > values[i-1] and values[i] > values[i+1]]
            dips = [i for i in range(1, len(values)-1) if values[i] < values[i-1] and values[i] < values[i+1]]
            return [{
                "period": best_lag, "correlation": round(best_corr, 3),
                "grain": grain,
                "peaks": [rows[i][0] for i in peaks],
                "dips": [rows[i][0] for i in dips],
            }]
        return []

    def _autocorr(self, x: List[float], lag: int) -> float:
        n = len(x)
        if n <= lag:
            return 0.0
        x_mean = sum(x) / n
        c0 = sum((xi - x_mean) ** 2 for xi in x) / n
        if c0 == 0:
            return 0.0
        c_lag = sum((x[i] - x_mean) * (x[i + lag] - x_mean) for i in range(n - lag)) / (n - lag)
        return c_lag / c0

    def query_ranking(self, dimension: str, measure: str,
                      grain: str, period_key: Optional[str] = None) -> List[dict]:
        """Rankings with change tracking."""
        pk = period_key or self._latest_period(grain)
        if not pk:
            return []
        rows = self.conn.execute(f"""
            ...
            WHERE d.grain     = ?
              ...
              AND d.dim_combo = ?
              AND d.measure   = ?
            ORDER BY dl.rank_in_period ASC
        """, [grain, pk, dimension, measure]).fetchall()
        return [{"label": r[0], "value": r[1], "rank": r[2], "prior_sum": r[3]}
                for r in rows if r[0] is not None]

    def stats(self) -> dict:
        row = self.conn.execute(f"SELECT COUNT(*) FROM {CUBE_TABLE}").fetchone()
        delta_row = self.conn.execute("SELECT COUNT(*) FROM axl_deltas").fetchone()
        stats_row = self.conn.execute("SELECT COUNT(*) FROM axl_stats").fetchone()
        return {
            "cube_cells":    row[0] if row else 0,
            "delta_cells":   delta_row[0] if delta_row else 0,
            "stat_groups":   stats_row[0] if stats_row else 0,
            "dims":          len(self.dims),
